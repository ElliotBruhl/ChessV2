...
public final class Board {
    //attributes
    private long[] bitboards = new long[12]; //indices: [0-w pawns, 1-b pawns, 2-w rooks, 3-b rooks, 4-w knights, 5-b knights, 6-w bishop, 7-b bishop, 8-w queen, 9-b queen, 10-w king, 11-b king]
    private boolean move; //true for white and false for black
    private int enPassantTarget = -1; //index of pawn than can be enPassanted
    private int halfmoves; //for 50 move rule
    private int fullmoves; //normal move counting
    private boolean[] castlingRights = new boolean[4]; //kingside w, queenside w, kingside b, queenside b
    private HashMap<Long, Integer> pastMoves = new HashMap<>(); //zorbist key, number of times seen before
    //constuctors
    public Board(String FEN) { //for initial/current board state
        readFEN(FEN);
    }
    private Board() {} //only used in copy method
    public Board newBoard() { //"this" refers to the old board and clone is only safe to use on collections of primative or immutable objects
        Board newBoard = new Board();
        newBoard.bitboards = this.bitboards.clone();
        ...
        newBoard.castlingRights = this.castlingRights.clone();
        newBoard.pastMoves = new HashMap<>(this.pastMoves);
        return newBoard;
    }
    //getters
    ...
    public boolean fastInCheck(boolean m) { //is current color in check
    ...
    //game management
    public Integer gameState() { //null = play on, 0 = draw, maxValue/2 = white win, minValue/2 = black win
        //stalemate and checkmate
        if (getLegalMoves().length == 0) { 
            return fastInCheck(this.move) ? (this.move ? Integer.MIN_VALUE/2 : Integer.MAX_VALUE/2) : 0;
        }
        ...
        (this.bitboards[0] | this.bitboards[1] | //no pawns
        this.bitboards[2] | this.bitboards[3] | //no rooks
        this.bitboards[8] | this.bitboards[9]) == 0L) { //no queens
        return 0;
        }
    ...
    public boolean manageMove(int start, int end) {
        ...
        switch (startBBIndex) {
            ...
            case 2 -> {
                if (start == 7) this.castlingRights[0] = false;
                else if (start == 0) this.castlingRights[1] = false;
            }
            case 3 -> {
                if (start == 63) this.castlingRights[2] = false;
                else if (start == 56) this.castlingRights[3] = false;
            }
            case 10 -> {
        ...
        if (Math.abs(end - start) == 16 && (startBBIndex == 0 || startBBIndex == 1)) {
            this.enPassantTarget = (start + end) / 2; // Midpoint for en passant target
        }
        ...
        if (capturedType != -1) { //irreversible move
            this.halfmoves = -1;
            this.pastMoves.clear();
            this.bitboards[capturedType] &= ~endMask; //remove captured piece
        }
        //move the piece and update board info
        this.bitboards[startBBIndex] &= ~startMask;
        this.bitboards[startBBIndex] |= endMask;
        this.pastMoves.merge(Zobrist.getHash(this), 1, Integer::sum); //adds (key, value) or increment value if present
        this.move = !this.move;
        this.halfmoves++;
        if (!this.move) this.fullmoves++;

        return promotion;
    }
    public void managePromotion(int index, int bbNum) { //bbNum: pass in -1 to choose piece (GUI), computer passes in 2, 4, 6, 8 for rook, knight, bishop, queen, respectively
        if (bbNum == -1) {
        ...
        long indexMask = (1L << (63-index));
        //promotion is managed after move change
        if (this.move) { //for black
            this.bitboards[bbNum + 1] |= indexMask;
            this.bitboards[1] &= ~indexMask;
        }
        else { //for white
            this.bitboards[bbNum] |= indexMask;
            this.bitboards[0] &= ~indexMask;
        }
    }
    //move generation
    public int[][] getLegalMoves() {
        //initialization info
        int[] starts = getStartIndices(this.move);
        ...
        long attackMask = getAttackMask(!this.move);
        ...
        for (int i = 0; i < starts.length; i++) {
            ...
            switch (getBBfromIndex(start)) {
                ...
                case 11 -> {
                    legalMoves |= (MagicBB.KingAttacks(start) & ~attackMask & ~friendly);
                ...
                default -> throw new Error("Bad BBIndex indicator");
            }
        }
        //format the results
        int moveCount = 0;
        for (long bb : allMoves) {
            moveCount += Long.bitCount(bb);
        }
        int[][] resultArr = new int[moveCount][2];
        moveCount = 0;
        for (int start = 0; start < starts.length; start++) {
            for (int end = 0; end < 64; end++) {
                if ((allMoves[start] & (1L << (63-end))) != 0) {
                    resultArr[moveCount][0] = starts[start];
                    resultArr[moveCount][1] = end;
                    moveCount++;
                }
            }
        }
    ...
    private long pawnCheck(int kIndex) {
        ...
        long pawnMask = 0L;
        switch (kIndex%8) { //so pawns cant attack past edges of board
            case 0 -> {
    ...
    private long getPinnersHV(int kIndex) { //makes mask of pin from king to pinning piece (including pinned piece)
        long oppRQPos = this.move ? (this.bitboards[3]|this.bitboards[9]) : (this.bitboards[2]|this.bitboards[8]);
        long blockers = getAllofColor(true)|getAllofColor(false);
        long xRayHV = MagicBB.xrayHV(kIndex);
        long possiblePinners = xRayHV&oppRQPos; //R/Q is lined up with king with unknown number of pieces in between
    ...
    private long getPinnersDiag(int kIndex) { //pretty much same as HV
        long oppBQPos = this.move ? (this.bitboards[7]|this.bitboards[9]) : (this.bitboards[6]|this.bitboards[8]);
        long blockers = getAllofColor(true)|getAllofColor(false);
    ...
    private long getAttackMask(boolean m) {
        ...
        long blockers = getAllofColor(true)|getAllofColor(false);
        blockers &= (m ? ~this.bitboards[11] : ~this.bitboards[10]);
        long attackMask = 0L;
        for (int start : getStartIndices(m)) {
            switch (getBBfromIndex(start)) {
                case 0-> { //wPawns
                    if (start%8 != 0)
                ...
                case 2, 3 -> { //Rooks
                    attackMask |= MagicBB.RookAttacks(63-start, blockers);
                }
                case 4, 5-> { //Knights
                    attackMask |= MagicBB.KnightAttacks(start);
                }
                case 6, 7 -> { //Bishops
                    attackMask |= MagicBB.BishopAttacks(63-start, blockers);
                }
                case 8, 9 -> { //Queens
                    attackMask |= MagicBB.QueenAttacks(63-start, blockers);
                }
                case 10, 11 -> { //Kings
    ...
    private void readFEN(String FEN) {
        ...
        catch (IndexOutOfBoundsException e) { //sometimes left off of FENS
            this.halfmoves = 0;
        }
